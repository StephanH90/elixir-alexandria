defmodule Alexandria.Test.InMemoryStorage do
  @moduledoc """
  Test-only in-memory implementation of `Alexandria.Storage`.

  Backed by a named Agent that is lazily started on first call.
  Compiled only under `test/support`, so it never ships with the library.
  """
